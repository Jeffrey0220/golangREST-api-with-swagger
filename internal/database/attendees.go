...
func (m *AttendeeModel) Delete(userId, eventId int) error {
	...
	query := "DELETE FROM attendees WHERE WHERE user_id = $1 AND event_id = $2"
...
func (m *AttendeeModel) GetEventsByAttendee(eventId int) ([]*Event, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	query := `SELECT e.id, e.owner_id, e.name, e.description, e.date, e.location 
	FROM events e JOIN attendees a ON e.id = a.event_id WHERE a.user_id = $1`
